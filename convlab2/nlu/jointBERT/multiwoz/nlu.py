...
class BERTNLU(NLU):
    def __init__(self, mode='all', config_file='multiwoz_all_context.json',
        ...
        dataloader = Dataloader(
            intent_vocab=intent_vocab, tag_vocab=tag_vocab,
        ...
        if (int(transformers.__version__.split('.')[0]) >= 3 and
                'bert.embeddings.position_ids' not in state_dict):
    ...
    def predict(self, utterance, context=list()):
        ...
        if self.use_context:
            if (len(context) > 0 and type(context[0]) is list and
                    len(context[0]) > 1):
        ...
        tag_seq = tag_seq[:510]
        batch_data = [
            [ori_word_seq, ori_tag_seq, intents, da, context_seq,
